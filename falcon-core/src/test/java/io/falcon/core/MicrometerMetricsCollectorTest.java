...
class MicrometerMetricsCollectorTest {
    ...
    void shouldRecordSuccessfulExecution() {
        ...
        Counter counter = registry.find("falcon.requests.success")
                .tag("action", "login")
                .counter();
        assertThat(counter).isNotNull();
        assertThat(counter.count()).isEqualTo(1.0);

        Timer timer = registry.find("falcon.response.time")
                .tag("action", "login")
                .timer();
    ...
    void shouldRecordFailedExecution() {
        ...
        Counter counter = registry.find("falcon.requests.failure")
                .tag("action", "checkout")
                .counter();
        assertThat(counter).isNotNull();
        assertThat(counter.count()).isEqualTo(1.0);
    ...
    void shouldTrackSuccessAndFailureSeparately() {
        ...
        Counter success = registry.find("falcon.requests.success")
                .tag("action", "api-call")
                .counter();
        Counter failure = registry.find("falcon.requests.failure")
                .tag("action", "api-call")
                .counter();
    ...
    void shouldRecordActiveUsers() {
        ...
        Gauge gauge = registry.find("falcon.active.users")
                .tag("action", "login")
                .gauge();
    ...
    void shouldTrackActiveUsersPerAction() {
        ...
        assertThat(registry.find("falcon.active.users")
    ...
    void shouldIsolateMetersPerAction() {
        ...
        Timer timerA = registry.find("falcon.response.time").tag("action", "action-a").timer();
        Timer timerB = registry.find("falcon.response.time").tag("action", "action-b").timer();
    ...
    void shouldHandleConcurrentRecording() throws Exception {
        ...
        Counter successCounter = registry.find("falcon.requests.success")
                .tag("action", "concurrent-action")
                .counter();
        Counter failureCounter = registry.find("falcon.requests.failure")
                .tag("action", "concurrent-action")
                .counter();
